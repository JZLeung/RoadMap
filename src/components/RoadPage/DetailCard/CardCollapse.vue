  ...
  <div id="Cardcollapse">
    ...
    <div id="content">
      ...
      <div class="dts" v-if="detail.deep_type == 'DINING'">
        <p>
          开业时间：<code>{{ detail.dining.opentime }}</code>
        </p>
        <p>
          打分：<code>{{ detail.dining.cp_rating }}</code>
        </p>
        <p>
          人均花费：<code>{{ detail.dining.cost }}</code> 元
        </p>
      </div>
      <!-- 景点信息 -->
      <div class="dts" v-if="detail.deep_type == 'SCENIC'">
        <p>
          开放时间：<code>{{ detail.scenic.opentime }}</code>
        </p>
        <p v-if="detail.scenic.rating">
          评分：<code>{{ detail.scenic.rating }}</code>
        </p>
        <p v-if="detail.scenic.price != 0">
          门票价格：<code>{{ detail.scenic.price }}</code> 元
        </p>
      </div>
      <!-- 酒店信息 -->
      <div class="dts" v-if="detail.deep_type == 'HOTEL'">
        <p>
          评分：<code>{{ detail.hotel.rating }}</code>
        </p>
        <p>
          星级：<code>{{ detail.hotel.star }}</code>
        </p>
        <p>
          最低价格：<code>{{ detail.hotel.lowest_price }}</code> 元
        </p>
      </div>
    </div>
  </div>
...
export default {
  ...
  mounted: function () {
    this.contentHeight =
      this.$el.clientHeight - this.$el.querySelector("#content").clientHeight;
  },
  computed: {
